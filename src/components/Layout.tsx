 ...
 */
const Layout: React.FC<{ children: React.ReactNode; onBlogClick: () => void }> = ({ children, onBlogClick }) => {
      ...
      <header className="bg-white shadow-sm border-b border-gray-200 sticky top-0 z-40 md:hidden">
        <div className="container mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center">
            ...
            <h1 className="text-xl font-bold text-gray-900 ml-2">Auxilio EBD</h1>
          </div>
        </div>
      </header>

      {/* Sidebar para desktop e menu mobile */}
      <div className={`fixed inset-y-0 left-0 z-50 w-64 bg-white shadow-lg transform ${isMenuOpen ? 'translate-x-0' : '-translate-x-full'} transition-transform duration-300 ease-in-out md:relative md:translate-x-0 md:shadow-none`}>
