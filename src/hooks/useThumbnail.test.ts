...
describe('useThumbnail', () => {
  ...
  it('should handle long title and description', async () => {
    ...
    expect(result.current).toMatch(/^data:image\/png;base64,/);
    // Additional check: ensure it's cached
    const cacheKey = `thumbnail_${btoa(longTitle + longDescription).slice(0, 50)}`;
    expect(localStorage.getItem(cacheKey)).toBe(result.current);
  });
